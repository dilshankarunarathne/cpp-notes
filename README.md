...
When we take `cout` as an input and return it back, when we have more than one objects to insert, those operations will be executed one by one. If we execute this `cout << c1 << c2` ,  
first `cout << c1`  will be executed. After that operation, the `cout` object is returned back to that point.  
Then we'd have `cout << c2` . That's why we need to return the `ostream` reference back to the caller. 
